...
public class DiscountByPercentStrategy implements IDiscountStrategy {
	 ...
	 * <p>Description: </p> 
	 * @return 
	 * @see com.pile.strategy.IDiscountStrategy#discount() 
	 */
	@Override
	public ChargeInfo discount(ChargeInfo chargeInfo, int discountType) {
		...
		try {
			Map<String, Object> discountMap = calculateFeeMapper.getDiscountBySerialNumber(serialNumber);
			if (null == discountMap) {
				Log4jUtils.getDiscountinfo().info("订单："+ serialNumber +" 未找到有效的折扣信息。");
				return chargeInfo;
			}
			
			/** 备用折扣标志 */
			int d2 = toInt(discountMap.get("d2"));
			/** 折扣标志 */
			int d1 = toInt(discountMap.get("d1"));
			...
			if (d2 == 1) {
				percentType = toByte(discountMap.get("discount_type2"));
				discountRate = toInt(discountMap.get("discount_rate2"));
				discountPrice = toInt(discountMap.get("discount_price2"));
			} else if (d1 == 1) {
				percentType = toByte(discountMap.get("discount_type1"));
				discountRate = toInt(discountMap.get("discount_rate1"));
				discountPrice = toInt(discountMap.get("discount_price1"));
			} else {
				return chargeInfo;
			}
			...
			if (percentType == Constant.DISCOUNTPERCENT) {
				...
				Map<String,Object> chargeDLMap = null;
				chargeDLMap = calculateFeeMapper.getChargeDLBySerialNumber(serialNumber);
				
				if (null == chargeDLMap) {
					Log4jUtils.getDiscountinfo().info("订单："+ serialNumber +" 未找到有效的订单信息。");
					return chargeInfo;
				}
				
				double chargeDL = toDouble(chargeDLMap.get("charge_dl"));
