...
public class DiscountByChargeFeeStrategy implements IDiscountStrategy {
	...
	*/
	...
	public ChargeInfo discount(ChargeInfo chargeInfo, int discountType) {
		...
		try {
			Map<String, Object> feeMap = calculateFeeMapper.getChargeFeeBySerialNumber(serialNumber);
			if (null == feeMap) {
			...
			/** 预充赠金 */
			int prechargeGive = toInt(feeMap.get("precharge_give"));
			/** 预充本金--分 */
			int prechargePrincipal = toInt(feeMap.get("precharge_principal"));
			/** 运营商扣费比例 */
			double payRatio = toDouble(feeMap.get("pay_ratio"));
			...
			/** 应退赠金 */
			int refundGive = prechargeGive - actualGive;
			/**应退本金*/
			int refundPrincipal = prechargePrincipal - actualPay;
			/** 更新账户余额  操作类型待议*/
			calculateFeeMapper.updateMemberAccount(refundGive, refundPrincipal, serialNumber);
