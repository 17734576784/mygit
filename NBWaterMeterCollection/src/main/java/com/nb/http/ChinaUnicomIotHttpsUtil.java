...
public class ChinaUnicomIotHttpsUtil extends DefaultHttpClient {
...
//		String demo_base_Path = "";//System.getProperty("user.dir");
		String selfcertpath =  ResourceUtils.getFile(Constant.CHINA_UNICOM_SELFCERTPATH).getAbsolutePath();
		String trustcapath = ResourceUtils.getFile(Constant.CHINA_UNICOM_TRUSTCAPATH).getAbsolutePath();
		
		KeyStore selfCert = KeyStore.getInstance("pkcs12");
		selfCert.load(new FileInputStream(selfcertpath),Constant.CHINA_UNICOM_SELFCERTPWD.toCharArray());
		KeyManagerFactory kmf = KeyManagerFactory.getInstance("sunx509");
		kmf.init(selfCert, Constant.CHINA_UNICOM_SELFCERTPWD.toCharArray());

		// 2 Import the CA certificate of the server,
		KeyStore caCert = KeyStore.getInstance("jks");
		caCert.load(new FileInputStream(trustcapath), Constant.CHINA_UNICOM_TRUSTCAPWD.toCharArray());
