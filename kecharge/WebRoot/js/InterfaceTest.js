...
function initSelect() {
	$("#partnerId").empty();
	$.post("partner/listParner.json", "", function(data) {
		if (data.status == 0) {
...
function keLoginSub(param) {
	...
	$("#keInterfa").css("display", "inline-block");
	var paramJson = {};
	paramJson.userName = partnerInfo[param - 1].loginKEuserName;
	paramJson.passWord = partnerInfo[param - 1].loginKEpassWord;
	//$("#jsonDataText").val(jsonString.json2String(paramJson).replace(/,/g,",\n"));
	$("#urlText").text("login.json");
