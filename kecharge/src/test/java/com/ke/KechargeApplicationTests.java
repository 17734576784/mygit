...
public class KechargeApplicationTests {
	...
	public void ChargeTest() throws Exception {
		...
		// Object tradeMsg = JedisUtil.brpop(Constant.CHARGE_START_QUEUE,
		// Constant.REDIS_TIMEOUT);

		// TradeMsgOuterClass.TradeMsgChargeBegin_Inf.Builder inf =
		// TradeMsgOuterClass.TradeMsgChargeBegin_Inf.newBuilder();
		// inf.setPaySerial("8800000054");
		// JedisUtil.set("ddd".getBytes(), inf.build().toByteArray());
		// byte[] t = tradeMsg.toString().getBytes();
		// for (byte b : t) {
		// System.out.print(b +" ");
		// }
		// System.out.println();
		// TradeMsgOuterClass.TradeMsgChargeBegin_Inf i =
		// TradeMsgOuterClass.TradeMsgChargeBegin_Inf.parseFrom(tradeMsg.toString().getBytes());
		// System.out.println(i.getPaySerial() +" "+ i.getFlag() +"
		// "+i.getReadings());

		// for (byte b : inf.build().toByteArray()) {
		// System.out.print(b +" ");
		// }
		// System.out.println();
		// System.out.println("*******************************");
		// Object test = inf.build().toByteArray();
		// byte[] tt = (byte[])test;
		// JedisUtil.set("dbr".getBytes(), tt);
		// for (byte b : tt) {
		// System.out.print(b +" ");
		// }

		// String key = Constant.GUNSTATE + 445;// + "_";
		// Set<String> keySet = JedisUtil.keysStr("gunstate:445_*");
		// for (String string : keySet) {
		// System.out.println(string);
		// }

		// List<Pilepara> list = this.pileMapper.listPileByOperatorId(1);
		// for (Pilepara pilepara : list) {
		// System.out.println(pilepara.getSerialCode());
		// }
		// OperatorConfig record = new OperatorConfig();
		// record.setOperatorId(1);
		// record.setOperatorLoginname("2343");
		// record.setMemberId(1);
		// record.setUseFlag((byte)1);
		// record.setUsername("xiaowo");
		// record.setPassword("xiaowo");
		// record.setSerialnumberPrefix("33");
		// record.setId(1);
		// record.setChargeAlramUrl("http://baidu.com");
		// record.setChargeDcInfoUrl("http://baidu.com");
		// record.setChargeHeartUrl("http://baidu.com");
		// record.setChargeOverUrl("http://baidu.com");
		// record.setChargeStartUrl("http://baidu.com");
		//
		// System.out.println(this.operatorConfigMapper.updateOperatorConfig(record));
		...
		// if (!flag) {
		// rtnJson = CommFunc.errorInfo(Constant.REQUEST_BAD, "充电站[" + stationNo
		// + "]不属于该运营商");
		// }
		...
		// MemberOrders order =
		// this.memberOrdersMapper.getmemberOrders("9999999999");
		...
		// String gunStateKey = Constant.GUNSTATE + orderMap.get("pileId") + "_"
		// + orderMap.get("gunId");
		...
		// SerializeUtil.deserialize(JedisUtil.get(key.getBytes()));
		//
	...
	public void listStationGPSTest() {
		...
		List<JSONObject> count = list.stream().filter(substpara -> substpara != null)
		...
		count = list.stream().parallel().filter(substpara -> substpara != null)
				.map(substpara -> getSubstpara(substpara)).collect(Collectors.toList());
		end = System.nanoTime();
