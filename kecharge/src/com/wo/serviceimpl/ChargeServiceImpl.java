...
public class ChargeServiceImpl implements IChargeService {
	...
	public JSONObject chargeStart(String queryJsonStr) {
		...
		json = JSONObject.fromObject(queryJsonStr);
		String pileNo = json.optString("pileNo");
		String gunNo = json.optString("gunNo");
		String chargeMoney = json.optString("chargeMoney");
		...
		if (pileNo.isEmpty() || gunNo.isEmpty() || payserialNumber.isEmpty()
			...
			return errorInfo(Constant.Err, "请求参数错误");
		}
		...
		if ((gunState < Constant.GUN_STATE_CHARGING && cpFlag == Constant.NOCP)
				|| (gunState == Constant.GUN_STATE_CONNECTION
						&& cpFlag == Constant.HAVECP && gunFlag == Constant.ELECTRIC_VEHICLE)
				|| (gunState < Constant.GUN_STATE_CHARGING
						&& cpFlag == Constant.HAVECP && gunFlag == Constant.ELECTRIC_BICYCLE)) {
			...
			YysCarownerOrder order = this.yysCarownerOrderMapper
					.getYysCarownerOrder(payserialNumber);
			if (null == order) {
			...
			// 向前置发送充电请求
			if (!sendRequest((byte) ComntDef.EVCP_WEBMSGTYPE_APP_CHARGE_PAY,
					rtuPara, task_data_vect, opDetail)) {
				rtnJson = errorInfo(Constant.Err, "请求太频繁,请稍后重试!");
				Log4jUtil.getChargeinfo().info(
						"web service 下发错误" + queryJsonStr);
			} else {
	...
	private boolean insertChargeOrder(String payserialNumber, String pileNo,
		...
		Map<String, Object> pileInfo = this.yysCarownerOrderMapper
				.getPileInfo(pileNo);
	...
	public ComntParaBase.RTU_PARA queryPileRtuPara(String pileCode) {
		...
		Map<String, Object> rtuMap = this.chargeMapper
				.queryRtuParaByPileNo(pileCode);
		if (rtuMap == null)
			return null;
	 ...
	 * 发送请求
	 * */
	public boolean sendRequest(byte requestType,
			ComntParaBase.RTU_PARA rtuPara,
	...
	public JSONObject chargeOver(String queryJsonStr) {
		...
		try {
			...
			// 将流水号转成12位
			paySerialNumber = autoGenericCode(paySerialNumber,
					Constant.PAY_SERIALNUMBER_LENGTH);
			...
			if (!sendRequest((byte) ComntDef.EVCP_WEBMSGTYPE_APP_CHARGE_PAY,
				...
				Log4jUtil.getChargeinfo()
						.info("webservice 下发失败" + queryJsonStr);
			} else {
				rtnJson = errorInfo(Constant.OK, "");
			}
			Log4jUtil.getChargeinfo().info(
					"流水号：" + paySerialNumber + "请求结束充电成功");
		} catch (Exception e) {
			Log4jUtil.getError().error("请求充电结束异常，请求内容：" + queryJsonStr, e);
	...
	public JSONObject getChargeData(String queryJsonStr) {
		...
		try {
			...
			if (chargeData != null
					&& objToInt(chargeData.get("charge_state")) == Constant.GUN_STATE_CHARGING) {

				chargTotalElectricity = objToStr(chargeData
						.get("charg_total_electricity"));
				chargHighElectricity = objToStr(chargeData
						.get("charg_high_electricity"));
				chargFlatElectricity = objToStr(chargeData
						.get("charg_flat_electricity"));
				chargLowElectricity = objToStr(chargeData
						.get("charg_low_electricity"));
				chargTipElectricity = objToStr(chargeData
						.get("charg_tip_electricity"));
			...
			Log4jUtil.getChargeinfo().info(
					"获取充电过程请求结束 ,返回结果: " + rtnJson.toString());
		} catch (Exception e) {
			Log4jUtil.getError().error("获取充电过程信息异常" + queryJsonStr, e);
	 ...
	 * 
	 * @see
	 * com.wo.service.IChargeService#SendChargOverRequest(net.sf.json.JSONObject
	 * )
	 */
	public boolean SendChargOverRequest(JSONObject json, int retry) {
		...
		Integer partnerId = chargeMapper
				.getPartnerIdBySerialNumber(paySerialNumber);
		if (null == partnerId) {
			return false;
		}
		...
		if (null == token || token.isEmpty()) {
			if (!WebConfig.initLoginToken(partnerId)) {
				return false;
			}
		}
		...
		try {
			param.put("queryJsonStr", java.net.URLEncoder.encode(sendJson
					.toString(), "UTF-8"));
			...
			Log4jUtil.getChargeinfo().info(
					"发送充电结束消息：" + json.toString() + " 接收内容： " + rtnJson);
			if (status == Constant.STATUS_OK
					|| status == Constant.STATUS_REQUEST_ERROR) {

				if (status == Constant.STATUS_REQUEST_ERROR) {
					Log4jUtil.getError().error(
							"丢弃充电结束消息推送,流水号：" + json.toString());
				}
				...
				Log4jUtil.getChargeinfo().info(
						"身份过期 重新获取token,发送内容：" + json.toString());
			...
			Log4jUtil.getError().error("发送充电结束请求异常,发送内容：" + json.toString(), e);
			flag = false;
			e.printStackTrace();
		}
	 ...
	 * 
	 * @see
	 * com.wo.service.IChargeService#SendChargStartRequest(net.sf.json.JSONObject
	 * )
	 */
	public boolean SendChargStartRequest(JSONObject json, int retry) {
		...
		try {
			...
			int status = jsonItem.getInt("status");
			Log4jUtil.getChargeinfo().info(
					"发送充电开始消息：" + json.toString() + " 返回结果 ： " + rtnJson);
			...
			Log4jUtil.getError().error("发送充电开始消息异常，发送内容：" + json.toString(), e);
			e.printStackTrace();
		}
	...
	public JSONObject getPileChargeRcd(String queryJsonStr) {
		...
		try {
			...
			String pileNo = json.optString("pileNo");
			String gunNo = json.optString("gunNo");
			if (paySerialNumber.isEmpty() || pileNo.isEmpty()
					|| gunNo.isEmpty()) {
				rtnJson = errorInfo(Constant.Err, "请求参数错误");
				return rtnJson;
			}
			...
			String chargeSerialNumber = yysCarownerOrderMapper
					.getChargeSerialNumber(paySerialNumber);
			...
			Map<String, Object> chargeRcd = this.chargeMapper
					.getPileChargeRcd(param);

			if (chargeRcd == null) {
				rtnJson = errorInfo(Constant.Err, "不存在【" + paySerialNumber
						+ "】的充电记录");
				return rtnJson;
			}

			StringBuffer startDate = new StringBuffer();
			startDate.append(FormatToYMD(chargeRcd.get("trade_bymd"), "day"))
					.append(" ");
			startDate.append(intToTime(chargeRcd.get("trade_bhms"), 1));

			StringBuffer endDate = new StringBuffer();
			endDate.append(FormatToYMD(chargeRcd.get("trade_eymd"), "day"))
					.append(" ");
			...
			Log4jUtil.getError().error("获取充电记录异常" + queryJsonStr, e);
	...
	public JSONObject getChargeRealData(String queryJsonStr) {
		...
		try {
			...
			Log4jUtil.getChargeinfo()
					.info("获取充电过程返回结果 : " + rtnJson.toString());
		} catch (Exception e) {
	 ...
	 * 
	 * @see
	 * com.wo.service.IChargeService#SendDCChargInfoRequest(net.sf.json.JSONObject
	 * )
	 */
	public boolean SendDCChargInfoRequest(JSONObject json, int retry) {
		...
		try {
			...
			int status = jsonItem.getInt("status");
			Log4jUtil.getChargeinfo().info(
					"发送直流充电信息消息：" + json.toString() + " 接收内容： " + rtnJson);
