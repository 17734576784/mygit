/**   
* @Title: ComntParaBase.java 
* @Package com.keicpms.communite 
* @Description: TODO(用一句话描述该文件做什么) 
* @author zhp
* @date 2018年12月13日 下午5:04:29 
* @version V1.0   
*/
...
public class ComntParaBase {
	...
	public static boolean get1StepTaskResult(String user_name, 
											 ...
											 byte[] task_result, 
											 ArrayList<ComntCfgEVCP.APP_CHARGE_RESULT> ret_data_vect, 
												   ...
												   StringBuffer opDetail) 
	{
		...
		if (!add_val) {
			detail_info.addTaskInfo("ERR:放入发送队列发生错误,请稍后重试！");
			opDetail.setLength(0);
			opDetail.append("放入发送队列发生错误,请稍后重试！");
			return false;
		}
		...
		if (++ferr_count >= 7200) {	//1h
			detail_info.addTaskInfo("ERR:等待超时,请重试！");			
			opDetail.setLength(0);
			opDetail.append("等待超时,请重试！");
			return false;
		}
