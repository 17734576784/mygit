...
public class ChinaUnicomCallBackController {
	@Resource
	private ChinaUnicomServiceContext chinaUnicomServiceContext;
	
	@Autowired
	private ChinaUnicomCommandContext chinaUnicomCommandContext;
...
//		}
	...
	public ResponseEntity<HttpStatus> recvDataChangeNotify(@RequestBody Object updateDeviceData_NotifyMessage) {
		...
		try {
			...
			chinaUnicomServiceContext.parseService(serviceId, deviceId, serviceMap);
		} catch (Exception e) {
...
//			} else
			if (resultCode.equals(Constant.COMMAND_SUCCESS)) {
				...
				if (!serviceName.isEmpty()) {
					...
					chinaUnicomCommandContext.parseCommand(serviceName, deviceId, commandMap);
					delCommand(commandId);
				}
